export const MobileMenu = ({ menuOpen, setMenuOpen }) => {
  ...
  return (
    <div
      className={`fixed inset-0 bg-black/95 backdrop-blur-lg z-30 transform transition-transform duration-300 ${
        menuOpen ? "translate-x-0" : "translate-x-full"
      } md:hidden flex flex-col justify-center items-center`}
    >
      <div className="flex flex-col items-center space-y-8">
        <MobileLink href="#home" onClick={handleLinkClick}>
          Home
        </MobileLink>
...
const MobileLink = ({ href, onClick, children }) => {
  return (
    <a
      href={href}
      onClick={onClick}
      className="text-2xl font-semibold text-white hover:text-blue-400 transition-colors duration-300 relative group"
    >
      {children}
      <span className="absolute -bottom-2 left-0 w-0 h-0.5 bg-gradient-to-r from-blue-500 to-cyan-400 transition-all duration-300 group-hover:w-full"></span>
    </a>
  );
};
